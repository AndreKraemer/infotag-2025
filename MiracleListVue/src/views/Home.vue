...
// Initialize HybridWebView composable to ensure message handlers are registered
const { isAvailable: isHybridAvailable, sendMessage } = useHybridWebView();
...
async function ShowTaskSet(c: Category | null | undefined) {
 ...
 data.category = c;
 if (c && c.categoryID) {
  AppState.CurrentCategoryID.value = c.categoryID;
...
async function TaskEditDone(changed: boolean) {
 ...
 if (changed && data.task) {
  ...
  // Use our convenience method to send messages
  sendMessage("Task Saved!");
